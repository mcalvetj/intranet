...
for (i = 0; i < contador; i++) {
    ...
    if (arrayjavascript_fecha_factura[i] == arrayjavascript_fecha_factura[i + 1]) {

        var x = parseFloat(arrayjavascript_precio_sin_iva[i]);
        var titulo = titulo + x;
        var texto = texto + arrayjavascript_nombre_cliente[i] + "  ->  " + arrayjavascript_precio_sin_iva[i] + "<br>"


    } else {

        fecha_factura = arrayjavascript_fecha_factura[i].split("-");
        any_factura = fecha_factura[0];
        mes_factura = fecha_factura[1];
        mes_factura = mes_factura - 1;
        dia_factura = fecha_factura[2];
        var x = parseFloat(arrayjavascript_precio_sin_iva[i]);
        var titulo = titulo + x;
        var texto = texto + arrayjavascript_nombre_cliente[i] + "  ->  " + parseFloat(arrayjavascript_precio_sin_iva[i]) + "<br>"

        //variable para el color del evento.
        var color_evento;
        if (titulo >= 0) {
            color_evento = 0;
        ...
        data.push({
            title: titulo.toFixed(2),
            start: new Date(any_factura, mes_factura, dia_factura),
            end: new Date(any_factura, mes_factura, dia_factura),
            allDay: color_evento,
            text: texto
        });
